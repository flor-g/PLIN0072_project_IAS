"""
Core scoring utilities used by the CLI entry points in `src/`.

This module contains:
- `Scorer`: base class that loads a HuggingFace causal LM + tokenizer.
- `SurprisalScorer`: token/word-level surprisal, entropy, and deviation scores.
- `IncrementalInformationValueScorer`: incremental alternative-sampling score based on
  distances between distributions of sampled continuations across timesteps.

Notes / suggested improvements:
- The word/token alignment utilities here are intentionally lightweight; they can be
  brittle for punctuation-heavy text or languages where whitespace isn't a word boundary.
- Long inputs currently raise `NotImplementedError` rather than truncating.
- Consider replacing ad-hoc CLI `eval()` parsing (in the scripts) with `ast.literal_eval()`.
"""
...
class Scorer:
    ...
    def __init__(self, model_name_or_path: str, device: Optional[str] = "cuda"):
        ...
        # NOTE: `padding_side="left"` matters for batched generation/scoring, but callers here mostly
        # feed single sequences. Some models/tokenizers may need an explicit `pad_token`.
    ...
    def get_word_character_positions(string):
        ...
        :param string (str):
            the input string
        """
        # NOTE: This is whitespace-based and ignores punctuation/tokenization conventions.
        # Suggested improvement: rely on tokenizer offsets exclusively (or use a real tokenizer).
        words = string.split()
        char_index_pairs = []
        pos = 0
        for w in words:
    ...
    def aggregate_score_by_word(self, string, scores, offsets, mode):
        # Aggregates per-token `scores` into per-word scores, using tokenizer-provided offsets.
        # This assumes the offsets line up cleanly with whitespace-split "words" above.
        try:
...
class SurprisalScorer(Scorer):
    ...
    def score(
        ...
        #
        # Note: offset mappings are only requested if we plan to aggregate by word, since some
        # tokenizers don't provide offset mappings unless explicitly asked.
        input_tokenized = self.tokenizer(input, return_offsets_mapping=aggregate_by_word, return_attention_mask=False)

        # Prepend BOS so we can score the first "real" token as a next-token prediction.
        # Suggested improvement: handle tokenizers without a BOS token (fallback to EOS or skip).
...
class IncrementalInformationValueScorer(Scorer):
    ...
    def __init__(
        ...
        """
        super().__init__(model_name_or_path, device)
        # `layers` is interpreted as indices into `outputs.hidden_states` (0..n_layer inclusive for many GPT-like models).
        # Suggested improvement: validate that requested layers exist for the loaded model.
    ...
    def embed_seq_alternatives(self, A: list, B: list, forecast_horizons: List[int], seq_len, length_true_seq_A,
        ...
        # Forward pass with all alternatives.
        #
        # Suggested improvement: batch A and B together (as in `embed_alternatives`) to reduce overhead.
        with torch.no_grad():
    ...
    def spearmanr_dist(A, B):
        # NOTE: This is O(n_alternatives^2) and can be very slow for large sets.
        # Suggested improvement: vectorize or restrict to a smaller sample for spearman distances.
    ...
    def map_words_to_token_positions(self, string, unit=1, tokenizer_offset_mapping=None, skip_first_token=False):
        ...
        if skip_first_token:
            # NOTE: This drops the first whitespace-split word (not the first token).
            # It's used to align timesteps when a BOS token is not added.
    ...
    def score(
        ...
        # Map unit boundaries (words or word-ngrams) to token positions; this defines the timesteps at
        # which we sample alternatives and compare their embedding distributions.
        ...
        for i, t in enumerate(timesteps):
            ...
            for i in range(n_sets):
                ...
                # Suggested improvement: pass an attention mask (some models behave better with it),
                # and consider using `temperature/top_p` explicitly for reproducible sampling configs.
